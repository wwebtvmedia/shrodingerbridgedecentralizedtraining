class LocalDatabase {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('neighbors')) {
                    const neighborStore = db.createObjectStore('neighbors', { keyPath: 'peerId' });
                    neighborStore.createIndex('timestamp', 'timestamp', { unique: false });
                    neighborStore.createIndex('loss', 'loss', { unique: false });
                }
                
                if (!db.objectStoreNames.contains('models')) {
                    const modelStore = db.createObjectStore('models', { keyPath: 'hash' });
    ...
    async cleanupOldNeighbors(maxAge = 7 * 24 * 60 * 60 * 1000) { // 7 days
        await this.ensureInitialized();
    ...
    async getStatistics() {
        ...
        return {
            neighbors: {
                ...
                active: neighbors.filter(n => Date.now() - n.updated < 5 * 60 * 1000).length, // Last 5 minutes
                bestLoss: neighbors.length > 0 ? Math.min(...neighbors.map(n => n.loss || Infinity)) : null
            },
            results: {
                ...
                averageLoss: results.length > 0 ? 
                    results.reduce((sum, r) => sum + (r.loss || 0), 0) / results.length : null
            },
            models: {
                count: models.length,
            ...
            request.onsuccess = () => {
                ...
                if (!type) {
                    ...
                    getAllRequest.onsuccess = () => {
                        const filtered = getAllRequest.result.filter(item => item.type === type);
    ...
    async importData(jsonData) {
        ...
        if (data.version === '1.1' && data.trainingData) {
            ...
            console.log(`📥 Imported ${data.trainingData.length} training data items`);
        }
        
        console.log(`📥 Imported ${data.neighbors?.length || 0} neighbors, ${data.results?.length || 0} results`);
    }
