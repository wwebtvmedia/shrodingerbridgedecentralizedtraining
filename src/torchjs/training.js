...
class OUReference {
  ...
  transitionKernel(z0, t, dt) {
    ...
    const var_ =
      (this.sigma ** 2 / (2 * this.theta)) * (1 - exp_neg_theta_dt ** 2);
    return [mean, var_];
  }
  ...
  bridgeSample(z0, z1, t) {
    ...
    // Mean
    const numerator =
      exp_neg_theta_t * (1 - exp_neg_theta_1_t ** 2) * z0 +
    ...
    let var_ =
      ((this.sigma ** 2 / (2 * this.theta)) *
        ((1 - exp_neg_theta_t ** 2) * (1 - exp_neg_theta_1_t ** 2))) /
      (1 - exp_neg_theta ** 2);
...
class KPITracker {
  ...
  computeConvergence() {
        ...
        const std =
          values.length >= window
            ? Math.sqrt(
                values
                  .slice(-window)
                  .reduce((sum, val) => sum + (val - ma) ** 2, 0) / window,
              )
            : 0;
    ...
    if (this.metrics.has("loss") && this.metrics.get("loss").length >= 10) {
      ...
      const lossStd = Math.sqrt(
        lossValues.reduce((sum, val) => {
          const mean =
...
export class EnhancedLabelTrainer {
  constructor(loader) {
    ...
    // OU reference process
    this.ou_ref = CONFIG.USE_OU_BRIDGE
      ? new OUReference(CONFIG.OU_THETA, CONFIG.OU_SIGMA)
      : null;
  ...
  computeLoss(batch, phase = 1, batchIdx = 0) {
    ...
    if (phase === 1) {
      ...
      const kl_annealing = Math.min(
        1.0,
        this.epoch / CONFIG.KL_ANNEALING_EPOCHS,
      );
      ...
      if (CONFIG.EDGE_WEIGHT > 0) {
        ...
        edge_loss = (edge_strength - real_edge_strength)
          .pow(2)
          .mul(CONFIG.EDGE_WEIGHT);
        recon_loss = recon_loss.add(edge_loss);
      }

      const total_loss = recon_loss
        .add(kl_loss)
        .add(diversity_loss * CONFIG.DIVERSITY_WEIGHT);
      ...
      const z1_noise = logvar
        .mul(0.5)
        .exp()
        .mul(Math.random() * temperature);
      ...
      if (
        this.epoch >
        CONFIG.TRAINING_SCHEDULE.switch_epoch + CONFIG.EPOCHS / 6
      ) {
        // Beta distribution
      ...
      // Start from noise
      const z0 = Array(z1.shape)
        .fill(0)
        .map(() => Math.random() * CONFIG.CST_COEF_GAUSSIAN_PRIO);
      ...
      // Time-weighted loss
      const t_reshaped = t.map((val) => [val, val, val, val]);
      const time_weights = t_reshaped.map(
        (t_val) => 1.0 + CONFIG.TIME_WEIGHT_FACTOR * t_val,
      );

      const drift_loss_base = pred
        .mul(time_weights)
        .sub(target.mul(time_weights))
        .abs()
        .mean()
        .mul(CONFIG.DRIFT_WEIGHT);

      const drift_start_epoch =
        this.phase2_start_epoch || CONFIG.TRAINING_SCHEDULE.switch_epoch;
      const consistency_decay = Math.max(
        0.1,
        1.0 -
          (this.epoch - drift_start_epoch) /
            (CONFIG.EPOCHS - drift_start_epoch),
      );

      if (phase === 3) {
        // Phase 3: Also train VAE reconstruction
        const recon_p3 = this.vae.decode(mu, labels, source_id);
        const recon_loss_p3 = images
          .sub(recon_p3)
          .abs()
          .mean()
          .mul(CONFIG.RECON_WEIGHT * CONFIG.PHASE3_RECON_SCALE);

        const total_loss = drift_loss_base
          .add(
            consistency_loss.mul(CONFIG.CONSISTENCY_WEIGHT * consistency_decay),
          )
          .add(recon_loss_p3);
        ...
        // Phase 2
        const total_loss = drift_loss_base.add(
          consistency_loss.mul(CONFIG.CONSISTENCY_WEIGHT * consistency_decay),
        );
