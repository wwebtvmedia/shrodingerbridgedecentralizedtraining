...
export class EnhancedLabelTrainer {
  ...
  async trainStep(batch, labels) {
    ...
    if (this.phase === 1) {
      ...
      return { 
        ...
        metrics: { phase: 'vae', recon_loss: recon_loss._data[0], kl_loss: kl_loss._data[0] } 
      };
    } else {
      ...
      return { 
        ...
        metrics: { phase: this.phase === 2 ? 'drift' : 'both', drift_loss: drift_loss._data[0] } 
      };
    }
  }
  ...
  loadCheckpoint(checkpoint) {
    if (checkpoint.vae_params) {
      ...
      checkpoint.vae_params.forEach((data, i) => { if (params[i]) params[i]._data = data; });
    }
    if (checkpoint.drift_params) {
      const params = this.drift.parameters();
      checkpoint.drift_params.forEach((data, i) => { if (params[i]) params[i]._data = data; });
    }
