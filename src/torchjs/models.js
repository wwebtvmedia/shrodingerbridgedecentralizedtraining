...
async function resolveTorch() {
  ...
  if (typeof window !== 'undefined') {
    console.log("🌐 Resolving torch in browser...");
    ...
    return new Promise((resolve, reject) => {
      ...
      const interval = setInterval(() => {
        if (window.torch) {
          ...
          console.log("✅ torch resolved from window");
          resolve(window.torch);
        }
        if (attempts++ > 100) {
          clearInterval(interval);
          reject(new Error("Timeout waiting for window.torch. Check if /js/js-pytorch-browser.js loaded."));
        }
      }, 100);
    });
  }
  ...
  // 2. Node.js check
  console.log("💻 Resolving torch in Node.js...");
  try {
    const JSTorch = await import('js-pytorch');
    const t = JSTorch.torch || (JSTorch.default && JSTorch.default.torch) || JSTorch;
    return t;
  } catch (e) {
    if (typeof globalThis !== 'undefined' && globalThis.torch) return globalThis.torch;
    throw e;
  }
}

// Ensure classes are defined after torch is ready
const torch = await resolveTorch();

if (!torch || !torch.nn) {
  console.error("❌ js-pytorch: torch.nn is missing!", torch);
