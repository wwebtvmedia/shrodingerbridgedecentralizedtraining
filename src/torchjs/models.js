...
class ResidualBlock {
    constructor(in_channels, out_channels, stride = 1) {
        ...
        if (in_channels !== out_channels || stride !== 1) {
            this.shortcut = createLayer('Conv2d', in_channels, out_channels, 1, stride, 0);
        } else {
            this.shortcut = null;
        }
    }
...
class SelfAttention {
    constructor(in_channels) {
        this.query = createLayer('Conv2d', in_channels, Math.floor(in_channels / 8), 1);
        this.key = createLayer('Conv2d', in_channels, Math.floor(in_channels / 8), 1);
    ...
    forward(x) {
        ...
        const q = this.query.forward(x).reshape([batch_size, Math.floor(channels / 8), height * width]);
        const k = this.key.forward(x).reshape([batch_size, Math.floor(channels / 8), height * width]);
        const v = this.value.forward(x).reshape([batch_size, channels, height * width]);
        
        // Attention
        const attn = q.transpose(1, 2).matmul(k).div(Math.sqrt(Math.floor(channels / 8)));
...
class LabelConditionedBlock {
    constructor(c_in, c_out, label_dim = CONFIG.LABEL_EMB_DIM, use_spectral_norm = false) {
    ...
    forward(x, labels = null) {
        ...
        if (labels !== null) {
            ...
            out = out.mul(scale.reshape([-1, out.shape[1], 1, 1])).add(shift.reshape([-1, out.shape[1], 1, 1]));
        }
...
export class LabelConditionedVAE {
    constructor(free_bits = CONFIG.FREE_BITS) {
        ...
        // Label embedding
        this.label_emb = createLayer('Embedding', CONFIG.NUM_CLASSES, CONFIG.LABEL_EMB_DIM);
        
        // Context embedding (if enabled)
        if (CONFIG.USE_CONTEXT) {
            this.source_emb = createLayer('Embedding', CONFIG.NUM_SOURCES, CONFIG.CONTEXT_DIM);
            this.cond_proj = createLayer('Linear', CONFIG.LABEL_EMB_DIM + CONFIG.CONTEXT_DIM, CONFIG.LABEL_EMB_DIM);
        }
    ...
    _channel_diversity_loss(mu) {
        ...
        const low_penalty = channel_stds.sub(target_std).abs().mul(CONFIG.DIVERSITY_LOW_PENALTY).mul(low_mask);
        const high_penalty = channel_stds.sub(max_std).mul(CONFIG.DIVERSITY_HIGH_PENALTY).mul(high_mask);
        
        return low_penalty.add(high_penalty).mean().mul(CONFIG.DIVERSITY_BALANCE_WEIGHT);
    }
    ...
    encode(x, labels, source_id = null) {
        ...
        if (this.training && Math.random() < CONFIG.CHANNEL_DROPOUT_PROB) {
            const channel_mask = torch.bernoulli(
                torch.full([mu.shape[0], mu.shape[1], 1, 1], CONFIG.CHANNEL_DROPOUT_SURVIVAL)
            );
    ...
    decode(z, labels, source_id = null) {
        ...
        if (h.shape[2] !== CONFIG.IMG_SIZE || h.shape[3] !== CONFIG.IMG_SIZE) {
            h = torch.nn.functional.interpolate(h, [CONFIG.IMG_SIZE, CONFIG.IMG_SIZE], 'bilinear');
        }
...
export class LabelConditionedDrift {
    constructor() {
        // Time embedding
        this.time_mlp = createLayer('Sequential',
        ...
        if (CONFIG.USE_CONTEXT) {
            this.source_emb = createLayer('Embedding', CONFIG.NUM_SOURCES, CONFIG.CONTEXT_DIM);
            this.cond_proj = createLayer('Linear', 256 + CONFIG.LABEL_EMB_DIM + CONFIG.CONTEXT_DIM, 128);
        } else {
            this.cond_proj = createLayer('Linear', 256 + CONFIG.LABEL_EMB_DIM, 128);
        }
        
        // Time-adaptive scaling
        this.time_weight_net = createLayer('Sequential',
        ...
        this.up2_conv = createLayer('Sequential',
        ...
        // Learnable scaling
        this.output_scale = torch ? new torch.nn.Parameter(torch.tensor(0.1)) : { data: 0.1 };
        this.time_scales = torch ? new torch.nn.Parameter(torch.ones(4).mul(0.1)) : { data: [0.1, 0.1, 0.1, 0.1] };
    ...
    forward(z, t, labels, cfg_scale = 1.0, source_id = null) {
        ...
        if (cfg_scale !== 1.0 && !this.training) {
            ...
            const uncond_drift = this._forward_internal(z, t, uncond_labels, t_emb, source_id);
    ...
    _forward_internal(z, t, labels, t_emb, source_id = null) {
        ...
        if (u2.shape[2] !== d1.shape[2] || u2.shape[3] !== d1.shape[3]) {
            u2 = torch.nn.functional.interpolate(u2, [d1.shape[2], d1.shape[3]], 'nearest');
        }
        ...
        // Scale output
        out = out.mul(this.output_scale).mul(time_weight.reshape([-1, 1, 1, 1]).add(1));
