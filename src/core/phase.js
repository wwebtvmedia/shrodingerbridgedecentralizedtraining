class PhaseManager {
    ...
    determinePhaseAdaptive(epoch, metrics) {
        ...
        if (this.currentPhase === 'vae' && this.shouldTransitionFromVAE(epoch, metrics)) {
            return 'drift';
        }
        
        if (this.currentPhase === 'drift' && this.shouldTransitionFromDrift(epoch, metrics)) {
            return 'both';
        }
    ...
    addRandomnessToWeights(amount) {
        for (const phase of Object.keys(this.phaseWeights)) {
            ...
            this.phaseWeights[phase] = Math.max(0.1, 
