...
export class ONNXExporter {
    ...
    async initialize() {
        ...
        try {
            ...
            console.warn('⚠️ Torch-js not available, ONNX export will generate schema only');
            this.torch = null;
        }
    }
    ...
    async exportVAEToONNX(vaeModel, outputPath = 'vae_model.onnx.json') {
        ...
        // Encoder layers
        this.addConvLayer(onnxModel, 'enc_in', 'input', 'enc_in_out', 3, 64, 3, 1, 1);
        this.addBatchNormLayer(onnxModel, 'enc_bn1', 'enc_in_out', 'enc_bn1_out', 64);
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this.addResidualBlock(onnxModel, `enc_block${i}`, `enc_act${i}_out`, `enc_block${i}_out`, inCh, outCh);
        }
        
        // Latent projections
        this.addConvLayer(onnxModel, 'z_mean', 'enc_block3_out', 'mu', 512, CONFIG.LATENT_CHANNELS, 1, 1, 0);
        this.addConvLayer(onnxModel, 'z_logvar', 'enc_block3_out', 'logvar', 512, CONFIG.LATENT_CHANNELS, 1, 1, 0);
        
        // Decoder layers
        this.addConvLayer(onnxModel, 'dec_in', 'mu', 'dec_in_out', CONFIG.LATENT_CHANNELS, 512, 1, 1, 0);
        
        for (let i = 0; i < 4; i++) {
            const inCh = [512, 512, 256, 128][i];
            const outCh = [512, 256, 128, 64][i];
            this.addResidualBlock(onnxModel, `dec_block${i}`, `dec_block${i-1 >= 0 ? i-1 : 'in'}_out`, `dec_block${i}_out`, inCh, outCh);
        }
        
        this.addConvLayer(onnxModel, 'dec_out', 'dec_block3_out', 'reconstruction', 64, 3, 3, 1, 1);
        this.addTanhLayer(onnxModel, 'final_tanh', 'reconstruction', 'reconstruction_tanh');
    ...
    async exportDriftToONNX(driftModel, outputPath = 'drift_model.onnx.json') {
        ...
        this.addLinearLayer(onnxModel, 'time_mlp1', 'time_emb_out', 'time_mlp1_out', 128, 256);
        this.addSiLULayer(onnxModel, 'time_mlp1_act', 'time_mlp1_out', 'time_mlp1_act_out');
        this.addLinearLayer(onnxModel, 'time_mlp2', 'time_mlp1_act_out', 'time_mlp2_out', 256, 256);
        
        // Label embedding
        this.addEmbeddingLayer(onnxModel, 'label_emb', 'labels', 'label_emb_out', CONFIG.NUM_CLASSES, CONFIG.LABEL_EMB_DIM);
        
        // Combine embeddings
        this.addConcatLayer(onnxModel, 'cond_concat', ['time_mlp2_out', 'label_emb_out'], 'cond_concat_out', -1);
        ...
        if (CONFIG.USE_CONTEXT) {
            ...
            this.addEmbeddingLayer(onnxModel, 'source_emb', 'source_id', 'source_emb_out', CONFIG.NUM_SOURCES, CONFIG.CONTEXT_DIM);
            this.addConcatLayer(onnxModel, 'cond_concat_full', ['cond_concat_out', 'source_emb_out'], 'cond_concat_full_out', -1);
            this.addLinearLayer(onnxModel, 'cond_proj', 'cond_concat_full_out', 'cond_out', 256 + CONFIG.LABEL_EMB_DIM + CONFIG.CONTEXT_DIM, 128);
        } else {
            this.addLinearLayer(onnxModel, 'cond_proj', 'cond_concat_out', 'cond_out', 256 + CONFIG.LABEL_EMB_DIM, 128);
        }
        
        // U-Net architecture (simplified)
        this.addConvLayer(onnxModel, 'head', 'z', 'head_out', CONFIG.LATENT_CHANNELS, 64, 3, 1, 1);
        this.addResidualBlock(onnxModel, 'down1', 'head_out', 'down1_out', 64, 128);
        this.addConvLayer(onnxModel, 'down2_conv', 'down1_out', 'down2_conv_out', 128, 256, 4, 2, 1);
        this.addResidualBlock(onnxModel, 'down2_block', 'down2_conv_out', 'down2_out', 256, 256);
        
        this.addResidualBlock(onnxModel, 'mid1', 'down2_out', 'mid1_out', 256, 256);
        this.addSelfAttention(onnxModel, 'mid_attn', 'mid1_out', 'mid_attn_out', 256);
        this.addResidualBlock(onnxModel, 'mid2', 'mid_attn_out', 'mid2_out', 256, 256);
        
        this.addUpsampleLayer(onnxModel, 'up2_upsample', 'mid2_out', 'up2_upsample_out', 2, 'nearest');
        this.addConvLayer(onnxModel, 'up2_conv', 'up2_upsample_out', 'up2_conv_out', 256, 128, 3, 1, 1);
        this.addResidualBlock(onnxModel, 'up2_block', 'up2_conv_out', 'up2_out', 128, 128);
        
        // Skip connection
        this.addAddLayer(onnxModel, 'skip_add', ['up2_out', 'down1_out'], 'skip_add_out');
        this.addResidualBlock(onnxModel, 'up1', 'skip_add_out', 'up1_out', 128, 64);
        
        this.addConvLayer(onnxModel, 'tail', 'up1_out', 'drift_raw', 64, CONFIG.LATENT_CHANNELS, 3, 1, 1);
        
        // Time-adaptive scaling
        this.addLinearLayer(onnxModel, 'time_weight_net1', 't', 'time_weight_net1_out', 1, 32);
        this.addSiLULayer(onnxModel, 'time_weight_net1_act', 'time_weight_net1_out', 'time_weight_net1_act_out');
        this.addLinearLayer(onnxModel, 'time_weight_net2', 'time_weight_net1_act_out', 'time_weight_net2_out', 32, 1);
        this.addSigmoidLayer(onnxModel, 'time_weight_sigmoid', 'time_weight_net2_out', 'time_weight');
        
        // Scale output
        this.addMulLayer(onnxModel, 'scale_output', ['drift_raw', 'time_weight'], 'drift');
    ...
    addConvLayer(onnxModel, name, input, output, inCh, outCh, kernel, stride, padding) {
        onnxModel.graph.node.push({
            ...
            attribute: [
                ...
                { name: 'pads', ints: [padding, padding, padding, padding], type: 'INTS' },
    ...
    addBatchNormLayer(onnxModel, name, input, output, numFeatures) {
        onnxModel.graph.node.push({
            ...
            input: [input, `${name}_scale`, `${name}_bias`, `${name}_mean`, `${name}_var`],
            output: [output],
            attribute: [
    ...
    addResidualBlock(onnxModel, name, input, output, inCh, outCh) {
        ...
        // First convolution
        this.addConvLayer(onnxModel, `${name}_conv1`, input, conv1Out, inCh, outCh, 3, 1, 1);
        this.addBatchNormLayer(onnxModel, `${name}_bn1`, conv1Out, bn1Out, outCh);
        this.addSiLULayer(onnxModel, `${name}_act1`, bn1Out, act1Out);
        
        // Second convolution
        this.addConvLayer(onnxModel, `${name}_conv2`, act1Out, conv2Out, outCh, outCh, 3, 1, 1);
        this.addBatchNormLayer(onnxModel, `${name}_bn2`, conv2Out, bn2Out, outCh);
        
        // Shortcut connection if needed
        if (inCh !== outCh) {
            this.addConvLayer(onnxModel, `${name}_shortcut`, input, shortcutOut, inCh, outCh, 1, 1, 0);
    ...
    addSelfAttention(onnxModel, name, input, output, channels) {
        ...
        onnxModel.graph.node.push({
            name,
            op_type: 'Attention',
            input: [input, `${name}_q_weight`, `${name}_k_weight`, `${name}_v_weight`],
            output: [output],
            attribute: [
                { name: 'num_heads', i: 1, type: 'INT' }
            ]
        });
    ...
    async extractWeightsToONNX(onnxModel, torchModel) {
        ...
        for (const initializer of onnxModel.graph.initializer) {
            if (!initializer.float_data) {
                ...
                initializer.float_data = Array.from({length: size}, () => Math.random() * 0.1);
            }
        }
    }
