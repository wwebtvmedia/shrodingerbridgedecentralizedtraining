class InferenceEngine {
  ...
  async loadModelsFromCheckpoint() {
    try {
      ...
      console.log(
        `📂 Loaded checkpoint from epoch ${checkpoint.metadata.epoch}`,
      );
  ...
  async generateSamples(options = {}) {
    ...
    this.currentInference.duration =
      this.currentInference.endTime - this.currentInference.startTime;
    ...
    console.log(
      `✅ Generated ${samples.length} samples in ${this.currentInference.duration}ms`,
    );
  ...
  async generateSampleWithSB(config, index) {
    ...
    for (let step = 0; step < steps; step++) {
      ...
      // Update progress
      if (this.currentInference) {
        this.currentInference.progress =
          (step / steps) * 50 + (index / (config.sampleCount || 4)) * 50;
  ...
  getInferenceStats() {
    ...
    const totalSamples = this.inferenceHistory.reduce(
      (sum, inf) => sum + inf.samples.length,
      0,
    );

    const totalTime = this.inferenceHistory.reduce(
      (sum, inf) => sum + (inf.duration || 0),
      0,
    );
