class DataImporter {
    ...
    async importText(files) {
        ...
        for (const file of files) {
            if (!this.supportedTextFormats.includes(file.type) && 
        ...
        try {
            ...
            for (const text of dataset.texts) {
                await database.saveResult({
                    ...
                    data: {
                        preview: typeof text.content === 'string' ? 
            ...
            console.log(`✅ Saved ${dataset.images.length} images and ${dataset.texts.length} texts to database`);
            return true;
    ...
    getImportStats(dataset) {
        return {
            ...
            imageStats: {
                totalPixels: dataset.images.reduce((sum, img) => sum + (img.width * img.height), 0),
                avgDimensions: this.calculateAvgDimensions(dataset.images)
            },
            textStats: {
                totalTokens: dataset.texts.reduce((sum, text) => sum + text.tokens.tokens, 0),
                avgTokens: dataset.texts.length > 0 ? 
                    dataset.texts.reduce((sum, text) => sum + text.tokens.tokens, 0) / dataset.texts.length : 0
            }
        };
    }
