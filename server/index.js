...
class ModelConsolidationServer {
    ...
    loadInitialModel() {
        try {
            if (fs.existsSync(this.latestModelPath)) {
                ...
                console.log(`✅ Loaded initial model: ${this.latestModelPath} (${stats.size} bytes)`);
            } else {
                console.log('⚠️  No initial model found. Waiting for client submissions.');
            }
        } catch (error) {
    ...
    setupRoutes() {
        ...
        this.app.post('/api/model/submit', async (req, res) => {
            try {
                ...
                console.log(`📥 Model submission from client ${clientId}: loss=${loss}, epoch=${epoch}`);
        ...
        this.app.get('/api/clients', (req, res) => {
            const clients = Array.from(this.trainingClients.entries()).map(([id, data]) => ({
                id,
                ...data,
    ...
    handleWebSocketMessage(clientId, message) {
        ...
        switch (message.type) {
            case 'register_training':
                ...
                console.log(`🏋️  Training client registered: ${clientId} (${message.data.name || 'unnamed'})`);
                break;
    ...
    async handleClientModelUpdate(clientId, message) {
        ...
        console.log(`📤 Model update from ${clientId}: loss=${loss}, epoch=${epoch}`);
    ...
    async evaluateModel(submission) {
        ...
        if (isBetter) {
            console.log(`🏆 New best model from ${clientId}: loss=${loss} (previous: ${this.bestModel?.loss || 'none'})`);
            
            // Save model to file
            const modelPath = path.join(this.modelsDir, `model_${Date.now()}_${clientId}.pt`);
    ...
    broadcastNewBestModel() {
        ...
        console.log(`📢 Broadcast new best model: loss=${this.bestModel.loss}, client=${this.bestModel.clientId}`);
    }
