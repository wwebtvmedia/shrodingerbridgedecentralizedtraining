...
async function testInferenceUpdates() {
  ...
  try {
    ...
    const vaeSchema = await onnxExporter.exportVAEToONNX(
      null,
      "test_vae.onnx.json",
    );
    ...
    const driftSchema = await onnxExporter.exportDriftToONNX(
      null,
      "test_drift.onnx.json",
    );
  ...
  try {
    ...
    const labelResult = await inferenceEngine.generateWithLabel(5, {
      steps: 5,
    });
    console.log("   ✅ Label-based generation working");

    // Test unconditional generation
    const unconditionalResult = await inferenceEngine.generateUnconditional({
      steps: 5,
    });
  ...
  console.log("================");
  console.log(
    "The inference and ONNX dump updates have been successfully implemented.",
  );
  console.log("\nKey improvements:");
  ...
  console.log(
    "6. Enhanced inference features (label-based, unconditional, cache)",
  );
