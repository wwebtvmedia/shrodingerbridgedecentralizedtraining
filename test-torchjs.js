...
async function testTorchJSIntegration() {
    ...
    try {
        ...
        console.log(`   ❌ Torch-JS Trainer initialization failed: ${error.message}`);
    }
    ...
    try {
        ...
        console.log(`   ✅ ModelManager initialized with torch-js: ${modelManager.state.torchjs_initialized}`);
    } catch (error) {
    ...
    try {
        ...
        if (result.metrics.torchjs) {
            console.log(`   - Additional metrics:`, Object.keys(result.metrics).filter(k => !['phase', 'torchjs'].includes(k)));
        }
    } catch (error) {
    ...
    console.log('\n📊 Summary:');
    console.log('===========');
    console.log('Torch-JS implementation has been successfully integrated with the swarm system.');
    console.log('The system will use torch-js when available, falling back to simulation mode otherwise.');
